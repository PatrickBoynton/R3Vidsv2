...
const SliderControls = () => {
  ...
  const convertSecondsToDuration = (duration: number): any => {
    ...
    return <Box style={{display: 'flex'}}>
      <Typography color="secondary" sx={timerStyles}>{convertSecondsToDuration(Number(currentTime > 0 ? currentTime + 1: currentTime))}
      </Typography>
      <Slider value={currentTime} min={0} max={duration} onChange={handleSliderChange} valueLabelDisplay="auto" size="small"
      />
      <Typography color="secondary" sx={timerStyles}>{convertSecondsToDuration(Math.floor(duration))}</Typography>
    </Box>
}
